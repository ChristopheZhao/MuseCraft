...
状态：已完成（事件化持久化、轻量场景/资源写入）；遗留：trace/audit 后端暂未接入，其作用仅限审计/诊断（不承担记忆或全量轨迹存储，避免与记忆混淆；调试可选组件：`app/events/listeners.py::TraceLogListener`、`app/events/sinks.py::FileTraceSink`），如需专用后端可另行规划，不影响当前闭环。
...
## 2) 问题定位与难点
- 当前状态（最新）
  - AgentExecution 表已删除，Agent 不再直写 DB，改为事件发布。
  - Orchestrator 已去除同步 `persist_from_mas_wm`，完成/失败发布事件。
  - 持久化收敛：DataPersistenceService 只接受事件/终态摘要，不再读 MAS WM；handle_persistence_event 使用 SessionLocal 自建会话落最小投影。
  - 轨迹日志：可选文件 sink（调试），正式轨迹/审计后端待定。
- 偏差/难点
  - 仍需对白名单/禁止字段定稿，限制 DB 只存业务元数据/资源索引，不存运行轨迹。
  - 事件载荷需足够，避免监听器回退 WM；需定义必需字段。
  - 兼容层定位：确认前端/API 消费，决定保留或后续下线；迁移框架保留但精简。
...
- 事件驱动解耦
  ...
  - 轨迹日志监听器（可选，调试）订阅事件写文件；正式轨迹应由记忆模块/专用后端处理。
- 持久化路径调整
  - DataPersistenceService 仅接受事件/终态摘要，写最小元数据/资源索引；不读 MAS WM。
  - AgentExecution 已删除；审计通过事件→轨迹日志（调试可选）或最小 DB 摘要，DB 只保留 Task 状态与资源索引。
- 切换策略
...
- P1 基础设施落地（高优先）
  ...
  - [ ] 轨迹日志监听器：如需调试轨迹，可启用文件 sink；正式轨迹后端待接入。
...
- P3 持久化路径收敛（中优先）
  - [x] DataPersistenceService 仅接受事件/终态摘要写最小集，移除 WM 全量路径。
...
- P5 收尾与验证（低优先）
  ...
  - [ ] 回归测试：事件→持久化最小投影、任务状态查询、资源索引、轨迹写入/检索（更新/跳过 AgentExecution 引用）。
  - [ ] 文档更新：架构边界、数据流、运维指南（事件总线/存储配置、白名单/禁止字段）。
...
- **Database（审计/业务元数据）**：只保留业务查询/审计所需最小集，不承载运行轨迹。
  - Task：task_id/title/status/progress/current_step/error 摘要/输入输出摘要（概念/voice_plan/内容要素）/时间戳；资源索引引用 Resource。
  - Scene：scene_number/type/title/description/script_text/narrative/visual 描述/时间轴（start/duration/end）/生成参数摘要。
  - Resource：task_id/scene_id/filename/file_path|url/resource_type/必要元数据；仅记录引用与元数据，不存文件/轨迹。
  - 不允许写入：运行时迭代序列、tool 调用明细、WM 快照全文、大文本或音视频二进制。
