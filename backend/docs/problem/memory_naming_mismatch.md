...
- 长期记忆使用 `MemoryManager` 名称，但实际是长记忆存取实现（episodic/semantic），名称未体现“long term”，且容易被误解为“统一管理器”。
...
## 原因分析
- 命名历史导致的错位：WorkingMemoryService 已明确短期职责，但 MemoryManager 沿用早期命名，被误解为总管长/短期记忆。
...
- 文档同步：在规划文档中澄清短期/长期职责与命名，明确 MemoryManager 仅代表长记忆实现，避免被误认为统一管理器。
